...
Office 类 skill 的本地结构处理统一走 Go helper：

- `go run ./cmd/skilltool office unpack -in file.docx -out work/docx`
- `go run ./cmd/skilltool office pack -in work/docx -out output/file.docx`

默认不再在 `skills/` 目录下引入 Python、JavaScript、PowerShell、Swift 脚本和大体积资源；skill 以精简的 `SKILL.md` 为主。
